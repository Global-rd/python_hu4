from pprint import pprint

user_info = {
    "name": "Mike",
    "age": 25,
    "favourite_meals": ["pizza", "carbonara", "sushi"],
    "phone_contacts": {
        "Mary": "+36701234567",
        "Tim": "+36207654321",
        "Tim2": "+36304567321",
        "Jim": "+364005000"
    }
}
print(f"Teljes lista: {user_info["favourite_meals"]}")
#'''
pprint('1. Kérj be a felhasználótól 4 programozási nyelvet vesszővel elválasztva, ' \
'szóközök nélkül. Konvertáld a kapott stringet egy listává, és add hozzá a fenti dictionary-hez “skills” néven.')
#'''
#bekérés
programm_language = input("Adj meg 4 programozási nyelvet vesszővel elválasztva: ").split(",")
#hozzáadás
user_info["skills"] = programm_language
pprint(user_info)
print('2. Rendezd a favourite_meals lista elemeit abc szerinti növekvő sorrendbe.')
print('---------------')
#rendezés
user_info["favourite_meals"].sort()
pprint (user_info)
print('3. Printeld ki a favourite_meals lista utolsó előtti elemét')
print('---------------')
print(f"Utolsó előtti elem: {user_info['favourite_meals'][-2]}")
print('4. Adj hozzá egy “spaghetti” string-et ugyanehhez a listához.')
print('---------------')
#hozzáadás
user_info["favourite_meals"].append("spaghetti")
pprint (user_info)
print('5. Add hozzá a favourite_meals-hez az aktuális favourite_meals lista harmadik és negyedik elemét (nem az index-ét) újra.')
print('---------------')
#meglévő elemek hozzáadása
user_info["favourite_meals"].extend(user_info["favourite_meals"][2:4])
pprint(user_info)
print('6. Ezután töröld az így keletkezett duplikátumokat!')
print('---------------')
user_info["favourite_meals"] = list(set(user_info["favourite_meals"]))
#újra rendezem
user_info["favourite_meals"].sort()
pprint (user_info)
print('7. Cseréld fel a favourite_meals lista első és utolsó elemét!')
print('---------------')
user_info["favourite_meals"][0], user_info["favourite_meals"][-1] = user_info["favourite_meals"][-1], user_info["favourite_meals"][0]
pprint (user_info)
print('8. A “phone_contacts” dictionary-hez adj hozzá egy új elemet, tetszőleges névvel és telefonszámmal.')
print('---------------')
user_info["phone_contacts"]["John"] = "+36303631549"
pprint (user_info)
print('9. Tim és Tim2 ugyanazt az embert reprezentálják a “phone_contacts”-ban, viszont a "Tim" key mögött lévő telefonszám már nem él. Töröld ki a telefonkönyvből!')
print('---------------')
del user_info["phone_contacts"]["Tim"]
pprint (user_info)
print('10. Adj hozzá egy olyan új embert “phone_contacts”-hoz, akinek 2 telefonszáma is van!')
print('---------------')
user_info["phone_contacts"]["Anna"] = ["+36205486650", "+36707654321"]
pprint(user_info)
print("Extra 1: Printeld ki a “skills” lista utolsó 3 elemét ellentétes sorrendben!")
print('---------------')
print(f"Utolsó 3 elem ellentétes sorrendben: {user_info['skills'][-3:][::-1]}")
print('Extra 2: Most, hogy Tim-nek már csak 1 telefonszáma van, érdemes lenne átnevezni Tim2-t Tim-re!')
print('---------------')
#user_info["phone_contacts"]["Tim"] = user_info["phone_contacts"]["Tim2"]
#del user_info["phone_contacts"]["Tim2"]
#javítás
user_info['phone_contacts']['Tim'] = user_info['phone_contacts'].pop('Tim2')
print('---------------')
pprint(user_info)